...
var url = 'mongodb://' + hostname + ':27017/simpleimage';
var database = null;
...
var getTopID = function() {
    return topID;
}

var assignImageID = function(imageEntry) {
    imageEntry.id = topID;
    setTopID(topID + 1);
}
...
var addUser = function(userData, callback) {
    if (userData === undefined
        ...
        || userData.email === undefined) {
            callback({
                message: "Could not add user. Malformed input."
            }, null);
            return;
    }
    ...
    if (filteredUsername !== userData.username) {
        callback({
            message: "Could not add user. Username contains an inavlid character."
        }, null);
        return;
    }
    ...
    if (filteredEmail !== userData.email) {
        callback({
            message: "Could not add user. Email contains an inavlid character."
        }, null);
        return;
    }
    ...
    bcrypt.hash(userData.password, 10, function(err, hash) {
        if (err) {
            callback({
                message: "Could not add user. Error saving password."
            }, null);
            return;
        }
        ...
        insertUserIntoDatabase(newUserData, function (err, result) {
            if (err) {
                if (err.code === 11000) {
                    console.log("Error adding user. Username already exists.");
                    callback({
                        name: "DuplicateField",
                        field: "username"
                    }, null);
...
var getConfigFromDatabase = function(callback) {
    configCollection.find({app: "simpleimage"}).toArray(function(err, results) {
        // var configResult = (!err && results.length == 0) ? null : results[0];
        callback(err, results[0]);
    });
};

var applyConfigOptions = function() {
    getConfigFromDatabase(function(err, config) {
        if (err) {
            console.log("There was an error retrieving the config options from the database.");
            return;
        }
        topID = config.topID;
    });
...
var printCurrentConfig = function() {
    getConfigFromDatabase(function(err, config) {
        if (err) {
            console.log("There was an error retrieving the config options from the database.");
            return;
...
var insertNewConfig = function() {
    databaseConfig = getDefaultConfig();
    configCollection.insertOne(databaseConfig, function(err, result) {
        if (err) {
            console.log("Could not add config options to database.");
            console.log(err);
            return;
        }
        console.log("Successfully added these config options:");
...
var updateConfig = function() {
    getConfigFromDatabase(function(err, config) {
        ...
        config.topID = topID;
        configCollection.updateOne({_id: config._id}, config, function(err, result) {
            if (err) {
...
var findCommentsForUser = function(username, callback) {
    username = util.sanitizeText(username);
...
var loginUser = function(userData, callback) {
    ...
    userData.username = util.sanitizeText(userData.username);
...
var writeLogEntry = function(text, severity, callback) {
...
module.exports.getTopID = getTopID;
...
module.exports.getConfigFromDatabase = getConfigFromDatabase;
module.exports.printCurrentConfig = printCurrentConfig;
module.exports.insertNewConfig = insertNewConfig;
module.exports.applyConfigOptions = applyConfigOptions;
module.exports.updateConfig = updateConfig;
module.exports.setTopID = setTopID;
