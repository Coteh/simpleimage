...
module.exports = function(app, router) {
    ...
    router.get(["/", "/index.htm(l|)"], function(req, res, next) {
        ...
        handleSession(req.session, function(err, sessionObj) {
            ...
            res.render("index", {
                ...
                guestHasExtraFeatures: (sessionObj.unregisteredSessionID),
            });
        });
    });
    ...
    router.post("/change_password", limiter, function (req, res, next) {
        handleSession(req.session, async function (err, sessionObj) {
            ...
            databaseOps.findUser(userData.username, async function (err, results) {
                ...
                var passwordStrengthResult = owasp.test(req.body.newPassword);
    ...
    router.put("/transfer_images", function (req, res, next) {
        handleSession(req.session, function (err, sessionObj) {
            ...
            if (!(req.body.ids instanceof Array)) {
                routeError(next, {
                    message: "An array of image IDs must be provided for the transfer."
                });
                return;
            }
            ...
            userActions.transferGuestImagesToUser(sessionObj, ids)
    ...
    router.delete("/images", limiter, function (req, res, next) {
        handleSession(req.session, function (err, sessionObj) {
            if (err) {
                if (err.type === "userID") {
                    req.session.userID = undefined;
