...
module.exports = function(app, router) {
    ...
    app.get(["/", "/index.htm(l|)"], function(req, res, next) {
        ...
        handleSession(req.session, function(err, sessionObj) {
            if (err) {
                ...
                renderError(res, 500, "Currently logged in user no longer exists on the database.");
                return;
            }
    ...
    app.get("/images/:id.:ext", function(req, res, next) {
        ...
        ImageDatabase.findImage(id, function(err, imageEntry) {
            if (err || imageEntry.length == 0 || (req.params.ext !== undefined && util.extToMimeType(req.params.ext) !== imageEntry[0].mimetype)) {
                res.send("This image does not exist on the database.");
            } else {
                console.log(imageEntry[0]);
    ...
    app.get("/images/:id/", function(req, res, next) {
        ...
        handleSession(req.session, function (err, sessionObj) {
            ...
            if (id === undefined) {
                res.status(500).send("Image of this ID does not exist on the database.");
                return;
            }
            ImageDatabase.findImage(id, function (err, imageEntry) {
        ...
        handleSession(req.session, function (err, sessionObj) {
            ...
            ImageDatabase.findUser(username, function (err, result) {
                if (err) {
                    res.status(500);
                    if (req.query.type === "json") {
                        res.send(util.createJSONResponseObject("error", "Could not find user " + username + "."));
                    } else { //html
                        res.send("<div>Could not find user " + username + ".</div>");
                    }
                    return;
                }
        ...
        ImageDatabase.findCommentsForImage(id, function (err, result) {
            if (err) {
                ...
                if (req.query.type === "html") {
                    res.send("<div id='comments'>ERROR: Could not load comments for image of image ID " + id + ".</div>");
                } else { //json
                    res.send("{\"status\": \"error\", \"message\": \"Could not load comments for image of image ID " + id + ".\"}");
                }
                return;
            }
            ...
            if (result.length === 0) {
                ...
                if (req.query.type === "html") {
                    ...
                    res.send("{\"status\": \"success\", \"message\": \"There are currently no comments to display.\"}");
                }
                return;
            }
    ...
    app.get("/users/:username/comments", function(req, res, next) {
        ...
        ImageDatabase.findCommentsForUser(username, function (err, result) {
            if (err) {
                res.status(500);
                if (req.query.type === "html") {
                    res.send("<div id='comments'>ERROR: Could not load comments for user of user ID " + id + ".</div>");
                } else { //json
                    res.send("{\"status\": \"error\", \"message\": \"Could not load comments for user of user ID " + id + ".\"}");
                }
                return;
            }
        ...
        ImageDatabase.findUserByHiddenID(req.session.userID, function(err, result) {
            ...
            uploadFunc(req, res, function (err) {
                ...
                if (req.files.length === 0) {
                    res.send("{\"status\": \"error\", \"message\": \"nothing selected\"}");
                    res.end();
                    return;
                }
                var imageEntry = {
                ...
                ImageDatabase.addImage(imageEntry, function (err, result) {
                    if (result != null) {
                        ...
                        res.send("{\"status\": \"success\", \"message\": \"uploaded\", \"id\": \"" + result.ops[0].id + "\"}");
                    } else {
    ...
    app.post("/comment", function(req, res, next) {
        ...
        ImageDatabase.addComment({
            ...
            ImageDatabase.findUserByHiddenID(userHiddenID, function(err, result) {
                if (err) {
                    res.status(500).send("{\"status\": \"error\", \"message\": \"Comment has been posted but could not be displayed " +
