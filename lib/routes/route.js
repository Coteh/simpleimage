...
module.exports = function(app, router) {
        ...
        databaseOps.findImagesForUser(username, function(err, images) {
            ...
            images.forEach(function(imageData) {
                base64Images.push({
                    ...
                    data: Buffer.from(imageData.data.buffer).toString("base64"),
                    id: imageData.id
                });
            });
            res.render("user-images-view", {username, images: base64Images});
        });
    });
            ...
            uploadFunc(req, res, function (err) {
                ...
                databaseOps.addImage(imageEntry, function (err, result) {
                    if (result != null) {
                        ...
                        actionHistory.writeActionHistory("Upload", "Image", result.ops[0].id, username, {
                            imageAuthor: result.ops[0].username,
                            url: req.url
                        });
                        res.send({
                            status: "success",
    ...
    router.post("/comment", function(req, res, next) {
        ...
        databaseOps.addComment({
            ...
            actionHistory.writeActionHistory("Add", "Comment", comment.image_id, comment.username, {
    ...
    router.post("/register", function(req, res, next) {
        ...
        databaseOps.addUser(userData, function(err, result) {
            ...
            actionHistory.writeActionHistory("Register", "User", userData.username, userData.username, {
            ...
            databaseOps.findImage(id, function (err, imageEntry) {
                if (err || imageEntry.length == 0) {
                    ...
                    databaseOps.deleteImage(id, function(err, result) {
                        ...
                        actionHistory.writeActionHistory("Delete", "Image", id, sessionObj.user.username, {
                            imageAuthor: imageEntry[0].username,
                            url: req.url
                        });
                        res.status(200);
                        res.send({
