...
module.exports = function(app, router) {
    ...
    router.get("/images/:id.:ext", function(req, res, next) {
        ...
        databaseOps.findImage(id, function(err, imageEntry) {
            var serverImage = function(imageBuffer, id, lowRes) {
                res.status(200);
                res.append("Cache-Control", "public, max-age=2592000");
                ...
                if(id) {
                    if(lowRes) {
                        logger.info("Served low resolution image " + id + " by direct link");
                    } else {
                        logger.info("Served image " + id + " by direct link");
                    }
                } else {
                    logger.error(`Unsupported image type for low resolution, serving original image.`);
                }  
            }
            if (err) {
            ...
            if (imageEntry.length == 0 || (req.params.ext !== undefined && util.extToMimeType(req.params.ext) !== imageEntry[0].mimetype)) {
                ...
                if(req.query.res && req.query.res === "low") {
                    ...
                    if(mimeType === "image/jpeg") {
                        sharp(imageEntry[0].data.buffer).jpeg({quality: 40}).toBuffer().then((imageBuffer) => {
                            serverImage(imageBuffer, imageEntry[0].id, true);
                        }).catch(() => {
                            serverImage(imageEntry[0].data.buffer);
                        });
                    }
                    else if(mimeType === "image/png") {
                        sharp(imageEntry[0].data.buffer).png({quality: 40}).toBuffer().then((imageBuffer) => {
                            serverImage(imageBuffer, imageEntry[0].id, true);
                        }).catch(() => {
                            serverImage(imageEntry[0].data.buffer);
                        });
                        ...
                        sharp(imageEntry[0].data.buffer).metadata().then((info) => {
                            var resizeDimensions = Math.round(info.width/2);
                            gifResize({ width: resizeDimensions })(imageEntry[0].data.buffer).then((imageBuffer) => { 
                                serverImage(imageBuffer, imageEntry[0].id, true);
                            }).catch(() => {
                                serverImage(imageEntry[0].data.buffer);
                            });
                        ...
                        var bitmap = bmp.decode(imageEntry[0].data.buffer, true);
                        sharp(bitmap.data, { 
                            raw: { 
                                width: bitmap.width, 
                        ...
                        }).toFormat('png').png({ quality: 40 }).toBuffer().then((imageBuffer) => {
                            serverImage(imageBuffer, imageEntry[0].id, true);
                        }).catch(() => {
                            serverImage(imageEntry[0].data.buffer);
                        });
                    ...
                    serverImage(imageEntry[0].data.buffer, imageEntry[0].id, false);
                }
            }
        });
    });
