    ...
    router.delete("/images", function (req, res, next) {
        handleSession(req.session, function (err, sessionObj) {
            ...
            if (typeof req.body.ids === "object") {
                // Multiple IDs in an array
                databaseOps.findImages(req.body.ids, function(err, data) {
                    for (var i = 0; i < data.length; i++) {
                        if (sessionObj.user.username !== data[i].username) {
                            routeError(next, {
                    ...
                    // TODO actually delete these images
