...
module.exports = function(app, router) {
    ...
    var crsfProtection = csurf(csurfOptions);
            ...
            databaseOps.findUser(userData.username, async function (err, results) {
                if (err || results.length === 0) {
                    if (err) {
                        ...
                        logger.writeError("Could not find user '" + userData.username + "' in database");
                    }
                    routeError(next, {
                ...
                try {
                    ...
                    logger.writeError("Error authenticating old password of user", e);
                }
                ...
                databaseOps.changeUserPassword(user._id, req.body.newPassword)
                    .then((result) => {
                        logger.writeLog("User '" + userData.username + "' password has been changed.", logger.SeverityLevel.INFO);
                        actionHistory.writeActionHistory({
