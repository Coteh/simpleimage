...
module.exports = function (app, router) {
                ...
                cookie: {
                    secure:
                        ...
                        (process.env.NODE_ENV === "development" &&
                            process.env.USE_DEV_HTTPS === "true"),
                },
            })
        );
    } else {
    ...
    function errorHandler(err, req, res, next) {
        logger.error(
            "Router has encountered an error: " + (err.stack || "No stack found for this error.")
        );
        ...
        databaseOps.findImage(id, function (err, imageEntry) {
            ...
            if (
                ...
                (req.params.ext !== undefined &&
                    util.extToMimeType(req.params.ext) !== imageEntry[0].mimetype)
            ) {
                logger.error(
        ...
        handleSession(req.session, function (err, sessionObj) {
            if (err) {
                if (err.type === "userID") {
                    ...
                    return sendError(err, next, {
                        message:
                            "User cannot be found. Ensure user hasn't been deleted and try again.",
                        statusCode: 404,
                    });
                } else {
                    return sendError(err, next);
        ...
        handleSession(req.session, async function (err, sessionObj) {
            if (err) {
                if (err.type === "userID") {
                    req.session.userID = undefined;
                    err.message =
                        "There was an error posting comment. " +
                        "User could not be found. Ensure that user hasn't been deleted and try again.";
                }
                return sendError(err, next, {
                    errorID: "sessionUserNotFound",
    ...
    router.post("/register", format("json"), limiter, function (req, res, next) {
        ...
        if (!usernameCheckResult.valid) {
            switch (usernameCheckResult.error) {
                case usernameUtil.UsernameError.USERNAME_TOO_LONG:
                    logger.error(
                        `Could not register user ${req.body.username}. Username contains too many characters.`
                    );
                    sendError(next, {
                        statusCode: 400,
                ...
                default:
                    logger.error(
                        `Could not register user ${req.body.username}. Unknown error with username check.`
                    );
                    sendError(next, {
                        statusCode: 400,
        ...
        handleSession(req.session, async function (err, sessionObj) {
            if (err) {
                if (err.type === "userID") {
                    req.session.userID = undefined;
                    err.message =
                        "There was an error changing password for this user. " +
                        "User could not be found. Ensure that user hasn't been deleted and try again.";
                }
                return sendError(err, next, {
                    errorID: "sessionUserNotFound",
            ...
            databaseOps.findUser(userData.username, async function (err, results) {
                ...
                if (req.body.newPassword === req.body.oldPassword) {
                    return sendError(next, {
                        ...
                        message:
                            "Could not change password. New password is the same as the old password.",
                        fields: ["oldPassword", "newPassword", "newPasswordConfirm"],
                        statusCode: 400,
                    });
                }

                let passwordStrengthResult = passwordUtil.verifyPasswordStrength(
        ...
        handleSession(req.session, function (err, sessionObj) {
            if (err) {
                if (err.type === "userID") {
                    req.session.userID = undefined;
                    err.message =
                        "There was an error deleting image. " +
                        "User could not be found. Ensure that user hasn't been deleted and try again.";
                    return sendError(err, next, {
                        errorID: "sessionUserNotFound",
                        statusCode: 404,
