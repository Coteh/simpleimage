            ...
            if (req.query.type === "html") {
                const promises = comments.map(comment => new Promise((resolve, reject) => {
                    databaseOps.findImageAttributes(comment.imageID, (err, results) => {
                        if (err) {
                            logger.error(`Could not find image of ID ${comment.imageID} for user comments page, err: `, err);
                            reject(err);
                            return;
                        }
                        resolve(results);
                ...
                Promise.allSettled(promises)
                    .then(results => {
                        if (results.some(result => result.status === "rejected")) {
                            res.status(500);
                            res.send(commentUtil.generateCommentErrorHTML("IMAGE_ATTRIBS_ERROR"));
                            return;
                        }
