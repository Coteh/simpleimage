...
module.exports = function(app, router) {
    ...
    function errorHandler(err, req, res, next) {
        logger.writeError("An error occurred.\n" + (err.stack || "No stack found for this error."));
    ...
    function routeError(err, next, options) {
        ...
        errObj.statusCode = options.statusCode || err.statusCode || 500;
    ...
    function handleSession(session, callback) {
        var sessionObj = {
            unregisteredSessionID: session.unregisteredSessionID
        };
            ...
            databaseOps.findImage(id, function (err, imageEntry) {
                if (err || imageEntry.length == 0) {
                    ...
                    res.render("image-view", {
                        ...
                        user: sessionObj.user,
                        fromUrl: req.url,
                        unregisteredSessionOwnsImage: (sessionObj.unregisteredSessionID !== undefined && sessionObj.unregisteredSessionID === imageEntry[0].unregisteredSessionID)
                    });
    ...
    router.get("/users/:username/images", function(req, res, next) {
        ...
        databaseOps.findImagesForUser(username, function(err, images) {
            ...
            res.status(200);
            res.type("html");
            var base64Images = util.constructBase64ImageArray(images);
            var finalHTML = "<div id='user-images'>";
            base64Images.forEach(function(elem) {
    ...
    router.get("/your_images", function (req, res, next) {
        handleSession(req.session, function (err, sessionObj) {
            if (err) {
                routeError(err, next);
                return;
            ...
            // TODO handleSession should throw an error if no user logged in...?
            if (sessionObj.user === undefined) {
                if (!sessionObj.unregisteredSessionID) {
                    routeError(next, {
                        message: "You must be logged in to perform this action."
                    });
                    return;
                }
                databaseOps.findImagesForUnregisteredUser(sessionObj.unregisteredSessionID, function (err, images) {
                    if (err) {
                        routeError(next, {
                            message: "Images could not be found for unregistered user " + sessionObj.unregisteredSessionID + "."
                        });
                        return;
                    }
                    res.status(200);
                ...
                databaseOps.findImagesForUser(sessionObj.user.username, function (err, images) {
                    if (err) {
                        routeError(next, {
                            message: "Images could not be found for user " + sessionObj.user.username + "."
                        });
                        return;
                    }
                    res.status(200);
    ...
    router.post("/upload", function(req, res, next) {
        ...
        handleSession(req.session, function(err, sessionObj) {
            ...
            uploadFunc(req, res, function (err) {
                ...
                var unregisteredSessionID;
                if (username == null) {
                    if (!req.session.unregisteredSessionID) {
                        unregisteredSessionID = crypto.randomBytes(16).toString("hex");
                    } else {
                        unregisteredSessionID = req.session.unregisteredSessionID;
                    }
                    imageEntry.unregisteredSessionID = unregisteredSessionID;
                }
                ...
                databaseOps.addImage(imageEntry, function (err, result) {
                    if (result != null) {
                        ...
                        actionHistory.writeActionHistory({
                            type: "UPLOAD_IMAGE" + ((!username) ? "_UNREGISTERED" : ""),
                            item: result.ops[0].id,
                            username: (username) ? username : "unregistered user",
                            unregisteredSessionID: (!username) ? unregisteredSessionID : undefined,
                            ipAddress: req.ip,
                            info: {
                                request_url: req.url,
                        ...
                        if (username == null && !req.session.unregisteredSessionID) {
                            req.session.unregisteredSessionID = unregisteredSessionID;
                        }
                        res.send({
                            status: "success",
    ...
    router.put("/transfer_images", function (req, res, next) {
        handleSession(req.session, function (err, sessionObj) {
            if (err) {
                if (err.type === "userID") {
                    req.session.userID = undefined;
            ...
            if (req.body.ids === undefined || req.body.ids.length === 0) {
                routeError(next, {
                    message: "No image ids provided for transfer."
                });
                return;
            }

            if (typeof req.body.ids === "object") {
                // Transfer multiple images to registered user
                var ids = req.body.ids;
                userActions.transferGuestImageMultiToUser(sessionObj, ids, function (err, callback) {
                    if (err) {
                        routeError(err, next);
                        return;
                    }

                    logger.writeLog("Transferred multiple images to user " + sessionObj.user.username + ".", logger.SeverityLevel.INFO);
                    actionHistory.writeActionHistory({
                        type: "USER_TRANSFER_IMAGE_MULTI",
                        item: ids,
                        username: sessionObj.user.username,
                        unregisteredSessionID: sessionObj.unregisteredSessionID,
                        ipAddress: req.ip,
                        info: {
                            request_url: req.url,
                            author: sessionObj.user.username
                        }
                    }, function (err, result) {
                        if (err) {
                            console.error(err);
                ...
                // Transfer single image to registered user
                var id = req.body.ids;
                userActions.transferGuestImageToUser(sessionObj, id)
                    .then(function (result) {
                        var transferredImage = result.image;
                        logger.writeLog("Transferred image " + id + " to user " + sessionObj.user.username + ".", logger.SeverityLevel.INFO);
                        actionHistory.writeActionHistory({
                            type: "USER_TRANSFER_IMAGE",
                            item: id,
                            username: sessionObj.user.username,
                            unregisteredSessionID: sessionObj.unregisteredSessionID,
                            ipAddress: req.ip,
                            info: {
                                request_url: req.url,
                                author: transferredImage.username
                            }
                        }, function (err, result) {
                            if (err) {
                                console.error(err);
                        ...
                        res.send({
                            status: "success",
                            message: "Image of ID " + id + " transferred successfully."
                        });
                    })
                    .catch(function(err) {
                        routeError(err, next);
                ...
                routeError(next, {
                    message: "Invalid image ID provided for transfer."
                });
                return;
            }
        });
    ...
    router.delete("/images", function (req, res, next) {
        handleSession(req.session, function (err, sessionObj) {
            ...
            if (typeof req.body.ids === "object") {
                ...
                var ids = req.body.ids;
                userActions.userDeleteImages(sessionObj, ids, function(err, callback) {
                    if (err) {
                        routeError(err, next);
                        return;
                    }

                    logger.writeLog("Deleted multiple images.", logger.SeverityLevel.INFO);
                    actionHistory.writeActionHistory({
                        type: "USER_DELETE_IMAGE_MULTI" + ((!sessionObj.user) ? "_UNREGISTERED" : ""),
                        item: ids,
                        username: (sessionObj.user) ? sessionObj.user.username : "unregistered user",
                        unregisteredSessionID: (!sessionObj.user) ? sessionObj.unregisteredSessionID : undefined,
                        ipAddress: req.ip,
                        info: {
                            request_url: req.url,
                            author: (sessionObj.user) ? sessionObj.user.username : "unregistered user"
                        }
                    }, function (err, result) {
                        if (err) {
                            console.error(err);
                ...
                // Delete single image
                var id = req.body.ids;
                userActions.userDeleteImage(sessionObj, id, function (err, result) {
                    if (err) {
                        routeError(err, next);
                        return;
                    }

                    var deletedImage = result.deletedImage;
                    logger.writeLog("Deleted image " + id + ".", logger.SeverityLevel.INFO);
                    actionHistory.writeActionHistory({
                        type: "USER_DELETE_IMAGE" + ((!sessionObj.user) ? "_UNREGISTERED" : ""),
                        item: id,
                        username: (sessionObj.user) ? sessionObj.user.username : "unregistered user",
                        unregisteredSessionID: (!sessionObj.user) ? sessionObj.unregisteredSessionID : undefined,
                        ipAddress: req.ip,
                ...
                routeError(next, {
                    message: "Invalid image ID provided for deletion."
                });
                return;
            }
        });
