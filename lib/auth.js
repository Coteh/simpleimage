...
var hashPassword = function (password) {
    return new Promise(function (resolve, reject) {
        ...
        let preHashedPassword = preHashPassword(password);
        bcrypt.hash(preHashedPassword, 10, function (err, hash) {
            if (err) {
...
var authenticateUser = function (user, passwordAttempt) {
    return new Promise(function (resolve, reject) {
        ...
        let preHashedPasswordAttempt = preHashPassword(passwordAttempt);
        bcrypt.compare(preHashedPasswordAttempt, user.password, function (err, result) {
            if (err) {
...
module.exports.preHashPassword = preHashPassword;
