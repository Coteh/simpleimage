                ...
                callback(
                    {
                        status: "error",
                        message:
                            "An error occurred when uploading your image. Please try again later.",
                    },
                    null
                );
                return;
            ...
            var checkForImageEntriesCollection = function (collectionNames) {
                if (collectionNames.indexOf("image-entries") < 0) {
                    ...
                    imageEntryCollection
                        .dropIndex("expire_index")
                        .then(() => {
                            logger.info(
                                "expire_index for images collection dropped successfully. Creating new expire_index"
                            );
                        ...
                        .catch((err) => {
                            if (err.codeName === "IndexNotFound") {
                                logger.info(
                                    "expire_index for images collection not found. Creating new expire_index"
                                );
                                createIndexForImageEntriesCollection(imageEntryCollection);
                                finishInitialization();
                            } else {
            ...
            database
                ...
                .then((collections) => {
                    ...
                    if (collectionNames.indexOf("users") < 0) {
                        database
                            .createCollection("users", { collation: { locale: "en", strength: 2 } })
                            .then((collection) => {
...
const findImagesForUser = function (username, callback) {
    module.exports.findUser(username, (err, results) => {
        if (err) {
            return callback(new Error(`Error finding user ${username}`), null);
        }
        if (results.length === 0) {
        ...
        imageEntryCollection.find({ username }).toArray(function (err, results) {
            if (err) {
                logger.error(
                    "There was an error finding images for provided username: " + username
                );
                callback(err, null);
                return;
            }
        ...
        function (err, result) {
            if (err) {
                ...
                callback(
                    {
                        message:
                            "Could not write action history entry to database due to an error.",
                    },
                    null
                );
                return;
